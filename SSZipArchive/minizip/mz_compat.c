...
zipFile tzipOpen(const char *path, int append)
{
	zlib_filefunc64_def pzlib = mz_stream_os_get_interface();
	return tzipOpen2(path, append, NULL, &pzlib);
}

zipFile tzipOpen64(const void *path, int append)
{
	zlib_filefunc64_def pzlib = mz_stream_os_get_interface();
	return tzipOpen2(path, append, NULL, &pzlib);
}

zipFile tzipOpen2(const char *path, int append, const char **globalcomment,
	zlib_filefunc_def *pzlib_filefunc_def)
{
	return tzipOpen2_64(path, append, globalcomment, pzlib_filefunc_def);
}

zipFile tzipOpen2_64(const void *path, int append, const char **globalcomment,
	zlib_filefunc64_def *pzlib_filefunc_def)
{
	...
	zip = tzipOpen_MZ(stream, append, globalcomment);
...
int tzipOpenNewFileInZip5(zipFile file, const char *filename, const zip_fileinfo *zipfi,
...
int tzipWriteInFileInZip(zipFile file, const void *buf, uint32_t len)
{
	mz_compat *compat = (mz_compat *)file;
	int32_t written = 0;
...
int tzipCloseFileInZipRaw(zipFile file, uint32_t uncompressed_size, uint32_t crc32)
{
	return tzipCloseFileInZipRaw64(file, uncompressed_size, crc32);
}

int tzipCloseFileInZipRaw64(zipFile file, int64_t uncompressed_size, uint32_t crc32)
{
	mz_compat *compat = (mz_compat *)file;
	if (compat == NULL)
		return ZIP_PARAMERROR;
...
int tzipCloseFileInZip(zipFile file)
{
	return tzipCloseFileInZip64(file);
}

int tzipCloseFileInZip64(zipFile file)
{
	mz_compat *compat = (mz_compat *)file;
	if (compat == NULL)
		return ZIP_PARAMERROR;
...
int tzipClose(zipFile file, const char *global_comment)
{
	return tzipClose_64(file, global_comment);
}

int tzipClose_64(zipFile file, const char *global_comment)
{
	return tzipClose2_64(file, global_comment, MZ_VERSION_MADEBY);
}

int tzipClose2_64(zipFile file, const char *global_comment, uint16_t version_madeby)
{
	mz_compat *compat = (mz_compat *)file;
	int32_t err = MZ_OK;

	if (compat->handle != NULL)
		err = tzipClose2_MZ(file, global_comment, version_madeby);
...
{
	return tzipClose2_MZ(file, global_comment, MZ_VERSION_MADEBY);
}
...
unzFile tunzOpen(const char *path)
{
	return tunzOpen64(path);
}

unzFile tunzOpen64(const void *path)
{
	zlib_filefunc64_def pzlib = mz_stream_os_get_interface();
	return tunzOpen2(path, &pzlib);
}

unzFile tunzOpen2(const char *path, zlib_filefunc_def *pzlib_filefunc_def)
{
	return tunzOpen2_64(path, pzlib_filefunc_def);
}

unzFile tunzOpen2_64(const void *path, zlib_filefunc64_def *pzlib_filefunc_def)
{
	unzFile unz = NULL;
	void *stream = NULL;
...
int tunzGetGlobalInfo(unzFile file, unz_global_info* pglobal_info32)
{
	mz_compat *compat = (mz_compat *)file;
	...
	err = tunzGetGlobalInfo64(file, &global_info64);
	if (err == MZ_OK)
	{
...
int tunzGetGlobalInfo64(unzFile file, unz_global_info64 *pglobal_info)
{
	mz_compat *compat = (mz_compat *)file;
...
int tunzGetGlobalComment(unzFile file, char *comment, uint16_t comment_size)
{
	mz_compat *compat = (mz_compat *)file;
	const char *comment_ptr = NULL;
	int32_t err = MZ_OK;
...
int tunzOpenCurrentFile3(unzFile file, int *method, int *level, int raw, const char *password)
{
	mz_compat *compat = (mz_compat *)file;
...
int tunzOpenCurrentFile(unzFile file)
{
	return tunzOpenCurrentFile3(file, NULL, NULL, 0, NULL);
}

int tunzOpenCurrentFilePassword(unzFile file, const char *password)
{
	return tunzOpenCurrentFile3(file, NULL, NULL, 0, password);
}

int tunzOpenCurrentFile2(unzFile file, int *method, int *level, int raw)
{
	return tunzOpenCurrentFile3(file, method, level, raw, NULL);
}

int tunzReadCurrentFile(unzFile file, void *buf, uint32_t len)
{
	mz_compat *compat = (mz_compat *)file;
	int32_t err = MZ_OK;
	if (compat == NULL || len >= INT32_MAX)
...
int tunzCloseCurrentFile(unzFile file)
{
	mz_compat *compat = (mz_compat *)file;
	int32_t err = MZ_OK;
	if (compat == NULL)
...
int tunzGetCurrentFileInfo(unzFile file, unz_file_info *pfile_info, char *filename,
...
int tunzGetCurrentFileInfo64(unzFile file, unz_file_info64 * pfile_info, char *filename,
	uint16_t filename_size, void *extrafield, uint16_t extrafield_size, char *comment, uint16_t comment_size)
{
	mz_compat *compat = (mz_compat *)file;
	mz_zip_file *file_info = NULL;
...
int tunzGoToFirstFile(unzFile file)
{
	mz_compat *compat = (mz_compat *)file;
	if (compat == NULL)
		return UNZ_PARAMERROR;
...
int tunzGoToNextFile(unzFile file)
{
	mz_compat *compat = (mz_compat *)file;
	int32_t err = MZ_OK;
	if (compat == NULL)
...
int tunzLocateFile(unzFile file, const char *filename, unzFileNameComparer filename_compare_func)
{
	mz_compat *compat = (mz_compat *)file;
	mz_zip_file *file_info = NULL;
...
int tunzGetFilePos(unzFile file, unz_file_pos *file_pos)
{
	mz_compat *compat = (mz_compat *)file;
	int32_t offset = 0;
...
int tunzGoToFilePos(unzFile file, unz_file_pos *file_pos)
{
	mz_compat *compat = (mz_compat *)file;
...
int tunzGetFilePos64(unzFile file, unz64_file_pos *file_pos)
{
	mz_compat *compat = (mz_compat *)file;
	int64_t offset = 0;
...
int tunzGoToFilePos64(unzFile file, const unz64_file_pos *file_pos)
{
	mz_compat *compat = (mz_compat *)file;
	int32_t err = MZ_OK;
...
int32_t tunzGetOffset(unzFile file)
{
	return (int32_t)unzGetOffset64(file);
}

int64_t tunzGetOffset64(unzFile file)
{
	mz_compat *compat = (mz_compat *)file;
	if (compat == NULL)
		return UNZ_PARAMERROR;
...
int tunzSetOffset(unzFile file, uint32_t pos)
{
	return unzSetOffset64(file, pos);
}

int tunzSetOffset64(unzFile file, int64_t pos)
{
	mz_compat *compat = (mz_compat *)file;
	if (compat == NULL)
		return UNZ_PARAMERROR;
...
int tunzGetLocalExtrafield(unzFile file, void *buf, unsigned int len)
{
	mz_compat *compat = (mz_compat *)file;
	mz_zip_file *file_info = NULL;
	int32_t err = MZ_OK;
