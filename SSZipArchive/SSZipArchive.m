...
+ (BOOL)isFilePasswordProtectedAtPath:(NSString *)path {
	// Begin opening
	zipFile zip = tunzOpen(path.fileSystemRepresentation);
	if (zip == NULL) {
		return NO;
	}

	BOOL passwordProtected = NO;
	int ret = tunzGoToFirstFile(zip);
	if (ret == UNZ_OK) {
		do {
			ret = tunzOpenCurrentFile(zip);
			if (ret != UNZ_OK) {
				// attempting with an arbitrary password to workaround `unzOpenCurrentFile` limitation on AES encrypted files
				ret = tunzOpenCurrentFilePassword(zip, "");
				tunzCloseCurrentFile(zip);
			...
			ret = tunzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
			tunzCloseCurrentFile(zip);
			if (ret != UNZ_OK) {
				break;
	...
	if (ret == UNZ_OK) {
		do {
			if (pw.length == 0) {
				...
				ret = tunzOpenCurrentFilePassword(zip, [pw cStringUsingEncoding:NSUTF8StringEncoding]);
			}
			if (ret != UNZ_OK) {
			...
			if (ret != UNZ_OK) {
				...
				int readBytes = tunzReadCurrentFile(zip, buffer, (unsigned)MIN(10UL,fileInfo.uncompressed_size));
				if (readBytes < 0) {
	  ...
	  completionHandler:(void (^_Nullable)(NSString *path, BOOL succeeded, NSError * _Nullable error))completionHandler
{
	...
	tunzGetGlobalInfo(zip, &globalInfo);

	// Begin unzipping
	int ret = 0;
	ret = tunzGoToFirstFile(zip);
	...
	do {
		...
		@autoreleasepool {
			if (password.length == 0) {
				...
				ret = tunzOpenCurrentFilePassword(zip, [password cStringUsingEncoding:NSUTF8StringEncoding]);
			}
			...
			tunzGetCurrentFileInfo(zip, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
			...
			if (isDirectory && !fileIsSymbolicLink) {
				...
				int readBytes = tunzReadCurrentFile(zip, buffer, 4096);
				if (readBytes >= 0) {
					...
					while (fp) {
						...
						readBytes = tunzReadCurrentFile(zip, buffer, 4096);
						if (readBytes < 0) {
							// Let's assume error Z_DATA_ERROR is caused by an invalid password
							// Let's assume other errors are caused by Content Not Readable
							success = NO;
			...
			else
			{
				...
				int bytesRead = 0;
				while ((bytesRead = tunzReadCurrentFile(zip, buffer, 4096)) > 0)
				{
					buffer[bytesRead] = 0;
			...
			crc_ret = tunzCloseCurrentFile(zip);
...
- (BOOL)open
{
	...
	_zip = tzipOpen(_path.fileSystemRepresentation, APPEND_STATUS_CREATE);
	return (NULL != _zip);
}
...
- (BOOL)writeFolderAtPath:(NSString *)path withFolderName:(NSString *)folderName withPassword:(nullable NSString *)password
{
	...
	tzipWriteInFileInZip(_zip, buffer, 0);
	tzipCloseFileInZip(_zip);
	return error == ZIP_OK;
}
...
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(nullable NSString *)fileName compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
	...
	while (!feof(input) && !ferror(input))
	{
		...
		tzipWriteInFileInZip(_zip, buffer, len);
	}
...
- (BOOL)writeData:(NSData *)data filename:(nullable NSString *)filename compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
	...
	tzipWriteInFileInZip(_zip, data.bytes, (unsigned int)data.length);
...
int _zipOpenEntry(zipFile entry, NSString *name, const zip_fileinfo *zipfi, int level, NSString *password, BOOL aes)
{
	...
	return tzipOpenNewFileInZip5(entry, name.fileSystemRepresentation, zipfi, NULL, 0, NULL, 0, NULL, Z_DEFLATED, level, 0, -MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, password.UTF8String, aes, made_on_darwin, flag_base, 0);
}
